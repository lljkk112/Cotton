...
import numpy as np
import torch
#from PyQt5.QtCore.QByteArray import toFloat
...
class Results(SimpleClass):
    ...
    def __init__(
        self, orig_img, path, names, mag,boxes=None, masks=None, probs=None, keypoints=None, obb=None, speed=None,stand=0.354687*1020,UseExcel=True
    ) -> None:
        """
    ...
    def plot(
        ...
        if pred_boxes is not None :
            def tofloat(aaa):
                tensor_data = aaa
                float_data = tensor_data.cpu().item()
                return float("{:.3f}".format(float_data))



            x = round(self.orig_shape[0] * 0.03)

            annotator.text([x + 20, x + 20], str(len(pred_boxes)), txt_color=(255, 255, 0))
            leng = 0
            area=0
            area1=0
            info={150:[300,456],300:[100,274],200:[200,405]}#放大倍数：【比例尺（微米），对应的像素长度】

            #print(pred_boxes)
            for i in pred_boxes.xywh:

                #leng += math.sqrt(i[2] ** 2 + i[3] ** 2) / self.stand * self.mag#mag是放大倍数，stand代表一个单位对应的像素长度
                leng += math.sqrt(i[2] ** 2 + i[3] ** 2) *(info[self.mag][0]/info[self.mag][1])
                area+=(i[2]*i[3]) * (info[self.mag][0]/info[self.mag][1]) * (info[self.mag][0]/info[self.mag][1])*0.95#
                area1+=i[2] * i[3]
               # print(i[2] * i[3])

            box_count = len(pred_boxes.xywh)
            if box_count != 0:
                avg_length_text = str(leng / box_count)
                avg_area_text = str(tofloat(area / box_count))
                area_ratio_text = str(tofloat(area1 / self.orig_shape[0] / self.orig_shape[1]))

                annotator.text([x + 20, x + 45], avg_length_text, txt_color=(255, 255, 0))
                annotator.text([x + 20, x + 70], avg_area_text, txt_color=(255, 255, 0))
                annotator.text([x + 20, x + 100], area_ratio_text, txt_color=(255, 255, 0))
            result_name = str(filename).strip() if filename else ""
            if box_count != 0 and result_name:
                excel_path = Path(__file__).resolve().parents[2] / "analyseresult.xlsx"
                if excel_path.exists():
                    df_existing = pd.read_excel(excel_path)
                else:
                    df_existing = pd.DataFrame(columns=["name","数量","长度","面积","面积占比"])
                new_row = pd.DataFrame({
                    "name": [result_name],  # 注意：值必须是列表形式，即使只有一个值
                    "数量": [str(len(pred_boxes))],
                    "长度": [avg_length_text],
                    "面积": [avg_area_text],
                    "面积占比": [area_ratio_text]
                })
                df_updated = pd.concat([df_existing, new_row], ignore_index=True)
                tmp_excel_path = excel_path.with_name(f"{excel_path.stem}_tmp{excel_path.suffix}")
                df_updated.to_excel(tmp_excel_path, index=False)
                for replace_attempt in range(5):
                    try:
                        ...
                        if replace_attempt == 4:
                            pending_path = excel_path.with_name(
                                f"{excel_path.stem}_pending_{time.strftime('%Y%m%d_%H%M%S')}{excel_path.suffix}"
                            )
                            os.replace(tmp_excel_path, pending_path)
                            LOGGER.warning(
                                f"WARNING: {excel_path} is locked; saved latest Excel result to {pending_path}"
                            )
                        else:
                            time.sleep(0.5)
